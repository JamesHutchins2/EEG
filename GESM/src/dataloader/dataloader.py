...
class EEGDataProcessor:
    def __init__(self, raw_file_path, event_description, resample_factor=3):
        ...
        existing_channel_locations = pd.read_csv(pwd + "/src/dataloader/electrode_locations/old_points.csv")
        new_channel_locations = pd.read_csv(pwd + "/src/dataloader/electrode_locations/new_points.csv")
    ...
    def load_raw_data(self):
        ...
        self.raw._data = concatenated_data
    def load_raw_without_interpolation(self):
        self.raw = mne.io.read_raw_brainvision(self.raw_file_path, preload=True)
        self.current_sfreq = self.raw.info["sfreq"]
        try:
            self.raw.crop(tmax=2000, tmin=1950)
        except:
            self.raw.crop(tmax=0.500, tmin=0.450)
        original_data = self.raw.get_data()
        
        self.raw._data = original_data
    ...
    def process_target_data(self):
        self.load_raw_without_interpolation()
        self.preprocess_raw_data()
        self.extract_epochs()
...
def create_dataset(raw_file_path, event_description, event_file_path, batch_size):
    
    eeg_processor = EEGDataProcessor(raw_file_path, event_description)
    eeg_processor.process_eeg_data()
    dataset = EEGDataLoader(eeg_processor.epochs, event_file_path)
    
    return dataset
def create_dataset_target(raw_file_path, event_description, event_file_path, batch_size):
    
    eeg_processor = EEGDataProcessor(raw_file_path, event_description)
    eeg_processor.process_target_data()
    dataset = EEGDataLoader(eeg_processor.epochs, event_file_path)
...
if __name__ == "__main__":
    ...
    event_file_name = '/mnt/a/MainFolder/Neural Nirvana/Data/sub-04/eeg/sub-04_task-rsvp_events.csv'
    eventDescription = 'Event/E  1'
    batch_size = 16
    train_split = 0.8
    dataset = create_dataset(raw_file_path, eventDescription,event_file_name, batch_size)
