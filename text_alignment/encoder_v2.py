...
class DecoderLayer(nn.Module):
    def __init__(self, input_dim=1024, decoder_embed_dim=1024, output_dim=512,
                 norm_layer=nn.LayerNorm):
        super(DecoderLayer, self).__init__()

        # Layers for the decoder
        self.linear1 = nn.Linear(input_dim, decoder_embed_dim * 2)
        self.norm1 = norm_layer(decoder_embed_dim * 2)
        self.activation1 = nn.LeakyReLU(0.01)

        self.linear2 = nn.Linear(decoder_embed_dim * 2, decoder_embed_dim // 2)
        self.norm2 = norm_layer(decoder_embed_dim // 2)
        self.activation2 = nn.LeakyReLU(0.01)

        # Flatten the sequence and feature dimensions before the final layer
        self.final_linear = nn.Linear((decoder_embed_dim // 2) * 129, output_dim)
    ...
    def forward(self, x):
        ...
        x = x.view(x.size(0), -1)  # Flatten keeping the batch size dimension

        # Final layer to produce the desired output dimension
...
class MaskedAutoencoder(nn.Module):
    ...
    def forward(self, eeg_input, text_token, target):
        ...
        pred = self.decoder_img.forward(latent)
        #print(pred.shape)
